...
async def Vrott_capture(bot, update):
    ...
    if "mxplayer" in update.text:
        try:
            w = update.text 
            req1 = requests.get("https://useraction.mxplayer.com/tokennd").json()
            rgx = re.findall("([0-9]?\w+)", w)[-3:]
            li = { "url":"mxplayervodnd.akamaized.net", "token":"https://gwapi.mxplayer.com/content/details/" }
            req2 = requests.get("https://useraction.mxplayer.com/token/platform_tokens.php?platform_name=web_app").json()["token"]
            headers["X-Access-Token"] = req2
            req3 = requests.get("https://useraction.mxplayer.com/token").json()    
            if "movies" in w:
        ...
        await update.reply_text("I can download from MX PLAYER links only! Use @Dads_links_bot for other links 😇", quote=True)
        return
